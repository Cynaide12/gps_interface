...
func DeleteGeofence(log *slog.Logger, coordinatesHandler CoordinatesHandler) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		const fn = "http-server.handlers.coordinates.AddGeofence"
		...
		err := coordinatesHandler.DeleteGeofence(req)
		if err != nil {
			log.Error("failed to add geofence", sl.Err(err))
			w.WriteHeader(http.StatusInternalServerError)
			render.JSON(w, r, response.Error("failed to add geofence"))
			return
		}
